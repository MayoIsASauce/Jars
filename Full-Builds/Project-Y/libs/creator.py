...
class _CreationOverseer:
    usedIds: list = []
    firstNames: str = ["Timothy", "John", "Marcus", "Matthew", "Todd", "Jeremy", "Lucas", "Brendan", "Colin", "Gregory"]
    lastNames: str = ["Doe", "O'Hanna", "Stevens", "Smith", "August", "Johnson", "Miller", "Jones", "Williams", "Anderson"]
...
class _Student:
    def __init__(self, Fname, Lname, Age, Year, Id = ""):
        ...
        self.email = "{inital}{Last}{Year}{ID}@sjvhs.com".format(inital=self.first[0].lower(), Last=self.last.lower(), Year=self.year, ID=self.ID)
    def __str__(self) -> str:
        return "{0}{1},{2},{3},{4},{5}\n".format(self.year, self.ID, self.last, self.first, self.age, self.email)
    first: str
    last: str
    age: int
    year: int
...
def generate(amnt: int) -> bool:
    ...
    if amnt > 3996 or len(_CreationOverseer.usedIds) >=  3996:
        return False
    for i in range(amnt):
        _fname = _CreationOverseer.firstNames[random.randint(0, 9)]
        _lname = _CreationOverseer.lastNames[random.randint(0, 9)]
